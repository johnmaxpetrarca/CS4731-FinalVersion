...
class Model {
    ...
    draw(gl, program, vBuffer) {
        if (!this.objParsed || !this.mtlParsed) {
            console.log("Can't draw model, not finished parsing OBJ or MTL");
            return;
        }

        // You would set up your attributes and uniforms for drawing here
        // This will likely include setting the vertex position, normal, and texture coordinate attributes,
        // as well as any uniforms needed for lighting and texturing.

        

        // Here's an example of how you might set up the vertex position attribute.
        // This assumes you've set up a vertex buffer and copied your vertex data to it.
        ...
        // Similar setup would be done for normal and texture coordinate attributes, and for any uniforms.

        // Draw each face of the model
        for(let face of this.faces) {
          // Here, you would bind the necessary buffers and set the appropriate uniforms
          // for each face before drawing it. This will probably depend on the material
          // associated with the face and whether the face is textured.
          ...
          // Then, draw the face
          gl.drawArrays(gl.TRIANGLES, 0, face.faceVertices.length / 3);
        }
    }
}
